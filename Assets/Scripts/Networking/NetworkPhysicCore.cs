...
public class NetworkPhysicCore : NetworkBehaviour
{
    ...
    public static void Solve(NetworkPhysicCore core, IBlock block, Vector3 force)
    {
    ...
    {
        NetworkPhysicCore NetworkPhysicCore = (NetworkPhysicCore)obj;
        while (NetworkPhysicCore.run)
        {
            if (NetworkPhysicCore.collideEvent.Count != 0)
            {
                openList.Clear();
                NetworkPhysicCore.locked = true;
                try
                {
                    foreach (KeyValuePair<IBlock, Vector3> cevent in NetworkPhysicCore.collideEvent)
                    {
                        Solve(NetworkPhysicCore, cevent.Key, Vector3.zero);
                        break;
                    }
                }
                catch
                ...
                NetworkPhysicCore.locked = false;
                NetworkPhysicCore.collideEvent.Clear();
                //CalculateRelativeForce(openList);
            }
            if (NetworkPhysicCore.dirty)
            {
                NetworkPhysicCore.CheckBlock();
                NetworkPhysicCore.dirty = false;
            }
            Thread.Sleep(deltaT);
        }
    }
    ...
    void ReCombind(List<IBlock> list)
    {
        ...
        rigidbody.centerOfMass /= rigidbody.mass;
        NetworkPhysicCore NetworkPhysicCore = newObj.GetComponent<NetworkPhysicCore>();
        NetworkPhysicCore.Load(list);
    }
