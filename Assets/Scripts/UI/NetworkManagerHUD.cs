...
public class NetworkManagerHUD : MonoBehaviour
{
    ...
    void DrawStatusGUI()
    {
        if (m_NetworkManager.IsServer)
        {
            ...
            GUILayout.Label($"{mode} active on port: {m_Transport.ConnectPort}", m_LabelTextStyle);
        }
        else
        {
            if (m_NetworkManager.IsConnectedClient)
            {
                GUILayout.Label($"Client connected {m_Transport.ConnectAddress}:{m_Transport.ConnectPort}", m_LabelTextStyle);
            }
        }
