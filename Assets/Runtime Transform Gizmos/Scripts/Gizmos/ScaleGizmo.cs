    ...
    public class ScaleGizmo : Gizmo
    {
        ...
        protected override void OnInputDeviceFirstButtonDown()
        {
            ...
            if (IsReadyForObjectManipulation())
            {
                ...
                {
                    _gameObjectLocalScaleSnapshot.Add(gameObject, gameObject.transform.parent.localScale);
                }
            }
        }
        ...
        private void DrawScaleBoxes(Matrix4x4[] worldTransforms)
        {
            ...
            worldTransform.SetTRS(_gizmoTransform.position,_gizmoTransform.rotation, 2*Vector3.Scale(_gizmoTransform.lossyScale, new Vector3(_scaleBoxWidth, _scaleBoxHeight, _scaleBoxDepth)));
            material.SetColor("_Color", Color.white);
            material.SetInt("_StencilRefValue", 255);
            material.SetPass(0);
            Graphics.DrawMeshNow(boxMesh, worldTransform);
        }
            ...
            if (ControlledObjects != null)
            {
                ...
                if(topParents.Count != 0)
                {
                    ...
                    foreach (GameObject topParent in topParents)
                    {
                        ...
                        {
                            // Cache object data
                            Transform objectTransform = topParent.transform.parent;
