...
namespace RTEditor
{
    ...
    public class RotationGizmo : Gizmo
    {
        ...
        [SerializeField]
        private bool _showFullRotationCircleX = false;
        [SerializeField]
        private bool _showFullRotationCircleY = false;
        [SerializeField]
        private bool _showFullRotationCircleZ = false;
        ...
        /// </summary>
        private float _accumulatedSnapRotation = 0.0f;

        private float _totalAccumulatedRotation = 0.0f;
        ...
        public bool ShowRotationSphere { get { return _showRotationSphere; } set { _showRotationSphere = value; } }
        ...
        public bool ShowFullRotationCircleX { get { return _showFullRotationCircleX; } set { _showFullRotationCircleX = value; } }
        public bool ShowFullRotationCircleY { get { return _showFullRotationCircleY; } set { _showFullRotationCircleY = value; } }
        public bool ShowFullRotationCircleZ { get { return _showFullRotationCircleZ; } set { _showFullRotationCircleZ = value; } }
        #endregion
        ...
        public override bool IsReadyForObjectManipulation()
        {
            return _selectedAxis != GizmoAxis.None ||
                   _isRotationSphereSelected || _isCameraLookRotationCircleSelected || DetectHoveredComponents(false);
        }
        ...
        protected override void OnInputDeviceFirstButtonDown()
        {
            ...
            if (_selectedAxis != GizmoAxis.None)
            {
                ...
                if (_selectedAxis == GizmoAxis.X) circlePlaneNormal = _gizmoTransform.right;
                else if (_selectedAxis == GizmoAxis.Y) circlePlaneNormal = _gizmoTransform.up;
            ...
            _accumulatedSnapRotation = 0.0f;

            _totalAccumulatedRotation = 0.0f;
        }
        ...
        protected override void OnInputDeviceMoved()
        {
            ...
            if (InputDevice.Instance.IsPressed(0))
            {
                ...
                if (_selectedAxis != GizmoAxis.None)
                {
                    ...
                    float mouseCursorMoveOffsetProjection = Vector2.Dot(screenSpaceTangent, InputDevice.Instance.GetDeltaSinceLastFrame(0));
                    ...
                    if(IsStepSnappingShActive)
                    {
                        ...
                        _accumulatedSnapRotation += relativeAngleInDegrees;

                        // If the absolute accumulated rotation value is >= to the snap step value, it means a rotation must be performed
                        if(Mathf.Abs(_accumulatedSnapRotation) >= _snapSettings.StepValueInDegrees)
                        {
                            ...
                            // calculated earlier.
                            float numberOfFullStepsInSnapRot = (float)((int)(Mathf.Abs(_accumulatedSnapRotation / _snapSettings.StepValueInDegrees)));
                            float rotationAngle = _snapSettings.StepValueInDegrees * numberOfFullStepsInSnapRot * Mathf.Sign(relativeAngleInDegrees);

                            // If the snap session was activated during a normal rotation session, we need to modify the rotation angle so that
                            // each snap rotation is relative to the zero rotation pose. Otherwise, it will be relative to the current accumulated rotation.
                            _totalAccumulatedRotation += rotationAngle;
                            float prevTotalRotation = _totalAccumulatedRotation;
                            float numberOfFullStepsInTotalRot = (float)((int)(Mathf.Abs(_totalAccumulatedRotation / _snapSettings.StepValueInDegrees)));
                            _totalAccumulatedRotation = numberOfFullStepsInTotalRot * _snapSettings.StepValueInDegrees * Mathf.Sign(_totalAccumulatedRotation);
                            rotationAngle -= (prevTotalRotation - _totalAccumulatedRotation);
                            ...
                            RotateControlledObjects(circlePlaneNormal, rotationAngle, (int)_selectedAxis);
                            ...
                            if (_accumulatedSnapRotation > 0.0f) _accumulatedSnapRotation -= _snapSettings.StepValueInDegrees * numberOfFullStepsInSnapRot;
                            else if (_accumulatedSnapRotation < 0.0f) _accumulatedSnapRotation += _snapSettings.StepValueInDegrees * numberOfFullStepsInSnapRot;
                        }
                    }
                    else
                    {
                        _totalAccumulatedRotation += relativeAngleInDegrees;
                        ...
                        RotateControlledObjects(circlePlaneNormal, relativeAngleInDegrees, (int)_selectedAxis);
                    }
                }
                else
                ...
                if (_isCameraLookRotationCircleSelected)
                {
                    ...
                    Vector2 cursorMoveOffsetSinceLeftMouseButtonDown = InputDevice.Instance.GetDeltaSincePressed(0);
                    ...
                    float accumulatedAngleInDegrees = InputDevice.Instance.GetDeltaSincePressed(0).magnitude * degreesPerScreenUnit * accumulatedMoveOffsetProjection;
                    ...
                    Vector2 mouseCursorMoveOffsetSinceLastFrame = InputDevice.Instance.GetDeltaSinceLastFrame(0);
                    ...
                    float relativeAngleInDegrees = InputDevice.Instance.GetDeltaSinceLastFrame(0).magnitude * moveOffsetProjection * degreesPerScreenUnit;

                    // Rotate the gizmo
                    _gizmoTransform.Rotate(_cameraTransform.forward, relativeAngleInDegrees, Space.World);
                    ...
                    RotateControlledObjects(_cameraTransform.forward, relativeAngleInDegrees);
                }
                else
                ...
                if (_isRotationSphereSelected)
                {
                    ...
                    float angleInDegreesAroundCameraRight = InputDevice.Instance.GetDeltaSinceLastFrame(0).magnitude * offsetProjectionOnUpTangent * degreesPerScreenUnit;
                    float angleInDegreesAroundCameraUp = InputDevice.Instance.GetDeltaSinceLastFrame(0).magnitude * offsetProjectionOnRightTangent * degreesPerScreenUnit;

                    // Apply the necessary rotations
                    _gizmoTransform.Rotate(_cameraTransform.right, angleInDegreesAroundCameraRight, Space.World);
                    _gizmoTransform.Rotate(_cameraTransform.up, angleInDegreesAroundCameraUp, Space.World);

                    // Make sure all controlled objects are rotated accordingly
                    RotateControlledObjects(_cameraTransform.right, angleInDegreesAroundCameraRight);
                    RotateControlledObjects(_cameraTransform.up, angleInDegreesAroundCameraUp);
                }
            }
        }
        ...
        protected override void OnRenderObject()
        {
            ...
            if (_showRotationSphere) DrawRotationSphere(GetRotationSphereWorldTransform());
            ...
            if (InputDevice.Instance.IsPressed(0))
            {
                ...
                if (_selectedAxis != GizmoAxis.None && _showRotationGuide)
                {
                    ...
                    if (_selectedAxis == GizmoAxis.Y) discPlaneNormal = _gizmoTransform.up;
                    else if (_selectedAxis == GizmoAxis.Z) discPlaneNormal = _gizmoTransform.forward;
            ...
            if (_axesVisibilityMask[0]) DrawRotationCircle(GizmoAxis.X);
            if (_axesVisibilityMask[1]) DrawRotationCircle(GizmoAxis.Y);
            if (_axesVisibilityMask[2]) DrawRotationCircle(GizmoAxis.Z);
        ...
        protected override bool DetectHoveredComponents(bool updateCompStates)
        {
            if(updateCompStates)
            {
                ...
                Ray pickRay = new Ray(Vector3.zero, Vector3.zero);
                bool canPick = InputDevice.Instance.GetPickRay(_camera, out pickRay);
                ...
                if (canPick)
                {
                    ...
                    for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
                    {
                        ...
                        bool usesFullCircle = (axisIndex == 0 && ShowFullRotationCircleX) ||
                                              (axisIndex == 1 && ShowFullRotationCircleY) ||
                                              (axisIndex == 2 && ShowFullRotationCircleZ);
                        ...
                        if (RayIntersectsRotationCircle(pickRay, circlePlaneNormals[axisIndex], circleRadius, out intersectionPoint))
                        {
                            ...
                            if (distanceFromCamera < minimumDistanceFromCamera)
                            {
                                ...
                                if (usesFullCircle || IsPointOnRotationCircleVisible(intersectionPoint, distanceFromCameraPositionToSphereCenter))
                                {
                ...
                float t;
                if (canPick && _selectedAxis == GizmoAxis.None && !_isCameraLookRotationCircleSelected && _showRotationSphere &&
                ...
                return _selectedAxis != GizmoAxis.None || _isCameraLookRotationCircleSelected || _isRotationSphereSelected;
            }    
            else
            {
                if (_camera == null) return false;
                ...
                if (canPick)
                {
                    ...
                    for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
                    {
                        ...
                        if (RayIntersectsRotationCircle(pickRay, circlePlaneNormals[axisIndex], circleRadius, out intersectionPoint))
                        {
                            ...
                            // Note: This check is necessary because when drawing the rotation circles, we will only draw the part of the
                            //       circle which is not obscured by the rotation sphere.
                            if (IsPointOnRotationCircleVisible(intersectionPoint, distanceFromCameraPositionToSphereCenter)) return true;
                        }
                    }
                }
                ...
                float t;
                if (canPick && _selectedAxis == GizmoAxis.None && !_isCameraLookRotationCircleSelected && _showRotationSphere &&
                    pickRay.IntersectsSphere(_gizmoTransform.position, GetWorldSpaceRotationSphereRadius(), out t)) return true;
        ...
        private bool IsGizmoInFrontOfCameraNearClipPlane()
        {
            ...
            Vector3 queryPoint = _gizmoTransform.position + _cameraTransform.forward * GetWorldSpaceRotationSphereRadius();
            Plane cameraNearPlane = new Plane(-_cameraTransform.forward, _cameraTransform.position + _cameraTransform.forward * _camera.nearClipPlane);
        ...
        private bool IsGizmoInFrontOfCameraFarClipPlane()
        {
            ...
            Vector3 queryPoint = _gizmoTransform.position - _cameraTransform.forward * GetWorldSpaceRotationSphereRadius();
            Plane cameraFarPlane = new Plane(_cameraTransform.forward, _cameraTransform.position + _cameraTransform.forward * _camera.farClipPlane);
        ...
        private void DrawRotationCircle(GizmoAxis gizmoAxis)
        {
            ...
            Color circleColor = (_selectedAxis == gizmoAxis ? _selectedAxisColor : _axesColors[(int)gizmoAxis]);
            bool useFullCircle = false;
            if (gizmoAxis == GizmoAxis.X && ShowFullRotationCircleX) useFullCircle = true;
            if (gizmoAxis == GizmoAxis.Y && ShowFullRotationCircleY) useFullCircle = true;
            if (gizmoAxis == GizmoAxis.Z && ShowFullRotationCircleZ) useFullCircle = true;
            ...
            for (int pointIndex = 0; pointIndex < worldSpaceCirclePoints.Length; ++pointIndex)
            {
                ...
                bool isPointVisible = useFullCircle || IsPointOnRotationCircleVisible(point, distanceFromCameraPositionToSphereCenter);
        ...
        private float EstimateRotationSphereScreenSpaceBoundaryCircleRadius(Vector3 screenSpaceBoundaryCircleCenter)
        {
            ...
            Vector3 pointOnTopOfCircleInScreenSpace = _gizmoTransform.position + _cameraTransform.up * GetWorldSpaceRotationSphereRadius();
        ...
        private float GetDistanceFromCameraPositionToRotationSphereCenter()
        {
            ...
            else return (_cameraTransform.position - _gizmoTransform.position).magnitude;
        }
        ...
        private bool IsPointOnRotationCircleVisible(Vector3 point, float distanceFromCameraPositionToSphereCenter)
        {
            ...
            if (_camera.orthographic)
            {
                Plane nearPlane = new Plane(_cameraTransform.forward, _cameraTransform.position + _cameraTransform.forward * _camera.nearClipPlane);
            ...
            else
            {
                ...
                Vector3 fromCameraPositionToPoint = point - _cameraTransform.position;
        ...
        private void RotateControlledObjects(Vector3 rotationAxis, float angleInDegrees, int rotationAxisIndex = -1)
        {
            if (ControlledObjects != null)
            {
                ...
                bool canUseAxisMask = (!_isCameraLookRotationCircleSelected && !_isRotationSphereSelected && rotationAxisIndex >= 0 && rotationAxisIndex < 3);
                    ...
                    else
                    {
                        ...
                        foreach (GameObject topParent in topParents)
                        {
                            ...
                            if (topParent != null)
                            {
                                ...
                                topParent.transform.parent.Rotate(rotationAxis, rotationAngle, Space.World);
                                IRTEditorEventListener editorEventListener = topParent.GetComponentInParent<IRTEditorEventListener>();
                                if (editorEventListener != null) editorEventListener.OnAlteredByTransformGizmo(this);
